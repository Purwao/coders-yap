          ...
          {post.frontMatter?.date && (
            <div className="flex items-center gap-2">
              ...
              <time dateTime={post.frontMatter.date}>
                {new Date(post.frontMatter.date.toString()).toLocaleDateString("en-US", {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric'
                })}
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {post.frontMatter.tags.map(tag => (
                    <span key={tag} className="bg-gray-800/50 px-2.5 py-1 rounded-full text-sm">
                      {tag}
                    </span>
                  ))}
                </div>
                    ...
                    <li key={`mobile-${heading.slug}`}>
                      <a
                        ...
                        className={`flex items-center py-1.5 hover:text-blue-400 transition-colors ${
                          heading.depth === 3 ? 'pl-6' : heading.depth === 2 ? 'pl-3' : ''
                        }`}
                      >
                        <span className={`inline-block w-1.5 h-1.5 rounded-full mr-3 ${
                          heading.depth === 1 ? 'bg-blue-500' : 
                          heading.depth === 2 ? 'bg-blue-400' : 'bg-blue-300'
                        }`}></span>
                        {heading.value}
                      </a>
                    </li>
          ...
          <div className="text-gray-300 space-y-6 text-base leading-relaxed">
            {post.mdxSource ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="mt-4 text-lg text-gray-400">No content available</p>
              </div>
            )}
          </div>
        </div>
              ...
              <div className="space-y-3 text-sm text-gray-400">
                ...
                {post.frontMatter?.tags && (
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {post.frontMatter.tags.map(tag => (
                        <span key={tag} className="bg-gray-800/50 px-2 py-1 rounded-full text-xs">
                          {tag}
                        </span>
                      ))}
                    </div>
