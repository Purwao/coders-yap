  ...
  return (
    <AnimatedCard key={`blog-post-${index}`}>
      <div
        ...
        className="h-full group relative bg-[#111827] border border-gray-800 rounded-2xl overflow-hidden shadow-md transition-transform duration-500 ease-[cubic-bezier(.2,.8,.2,1)] transform-gpu hover:-translate-y-2 hover:scale-[1.1] hover:shadow-blue-500/30 hover:border-blue-500"
      >
        <Link
          href={`/blog/${slug}`}>
          {/* Background Gradient */}
          ...
          <div className="p-4 flex flex-col gap-2 relative z-10">
            <div className="flex flex-wrap items-center gap-2 text-[11px] text-[#9ca3af] mb-1">
              ...
              {tags.map((tag) => (
                <span
                  key={tag}
                  className="bg-[#374151] text-white px-2 py-0.5 rounded-full"
                >
                  {tag}
                </span>
              ))}
